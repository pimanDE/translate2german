...
sudo rpl --encoding UTF-8 'enter a valid <a href="https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm" target="_blank">TOTP</a>' 'zusätzlich zu Ihrem Passwort ein gültiges Token <a href="https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm" target="_blank">TOTP</a> eingeben.' /var/www/html/admin/login.lp
...
sudo rpl --encoding UTF-8 'After installing Pi-hole for the first time, a password is generated and displayed' 'Nach der Erstinstallation von Pi-Loch wird ein Passwort generiert und dem Benutzer angezeigt.' /var/www/html/admin/login.lp
...
sudo rpl --encoding UTF-8 'Device does not use Pi-hole</td>' 'Gerät verwendet nicht Pi-hole</td>' /var/www/html/admin/network.lp
...
sudo rpl --encoding UTF-8 'Refresh</a>' 'Aktualisieren</a>' /var/www/html/admin/queries.lp
...
sudo rpl --encoding UTF-8 'Never forward reverse lookups for private IP ranges' 'Leite niemals Reverse-Lookups für private IP-Bereiche weiter' /var/www/html/admin/settings-dns.lp
...
sudo rpl --encoding UTF-8 'but may also prevent you from being able to access to access local hostnames if the Pi-hole is not used as DHCP server.' 'aber auch verhindern, dass Sie auf lokale Hostnamen zugreifen können, wenn Pi-hole nicht als DHCP-Server verwendet wird.' /var/www/html/admin/settings-dns.lp
...
sudo rpl --encoding UTF-8 'When a client makes too many queries in too short time, it' 'Wenn ein Gerät in zu kurzer Zeit zu viele Abfragen durchführt,' /var/www/html/admin/settings-dns.lp
...
sudo rpl --encoding UTF-8 'values to zero. See' 'indem beide Werte auf Null gesetzt werden. Siehe auch' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 '<a href="https://docs.pi-hole.net/ftldns/configfile/#rate_limit" target="_blank">our documentation</a>' '<a href="https://docs.pi-hole.net/ftldns/configfile/#rate_limit" target="_blank">unsere Dokumentation</a>' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'for further details.' 'für weitere Details.' /var/www/html/admin/settings-dns.lp
...
sudo rpl --encoding UTF-8 'The target of a <code>CNAME</code> must be a domain that the Pi-hole already has in its cache or is authoritative for. This is a universal limitation of <code>CNAME</code> records.' 'Das Ziel eines <code>CNAME</code> muss eine Domain sein, die Pi-Hole bereits in seinem Cache hat oder dafür maßgebend ist. Dies ist eine universelle Einschränkung von <code>CNAME</code>-Datensätzen. Der Grund dafür ist, dass Pi-hole bei der Bereitstellung von <code>CNAME</code>-Antworten' /var/www/html/admin/settings-dnsrecords.lp
sudo rpl --encoding UTF-8 "The reason for this is that Pi-hole will not send additional queries upstream when serving <code>CNAME</code> replies. As consequence, if you set a target that isn\'t already known, the reply to the client may be incomplete. Pi-hole just returns the information it knows at the time of the query. This results in certain limitations for <code>CNAME</code> targets," "keine zusätzlichen Anfragen im Upstream sendet. Wenn Sie ein Ziel festlegen, das noch nicht bekannt ist, ist die Antwort an das Gerät möglicherweise unvollständig. Pi-hole gibt lediglich die Informationen zurück, die es zum Zeitpunkt der Anfrage kennt. Dies führt zu bestimmten Einschränkungen für" /var/www/html/admin/settings-dnsrecords.lp
sudo rpl --encoding UTF-8 "for instance, only <i>active</i> DHCP leases work as targets - mere DHCP <i>leases</i> aren\'t sufficient as they aren\'t \(yet\) valid DNS records." "<code>CNAME</code>-Ziele. Beispielsweise funktionieren nur <i>aktive</i> DHCP-Vergaben als Ziele – bloße DHCP-<i>Vergaben</i> reichen nicht aus, da es sich (noch) nicht um gültige DNS-Einträge handelt. Darüber hinaus können Sie externe Domains (<code>bing.com</code> bis <code>google.com</code>) nicht" /var/www/html/admin/settings-dnsrecords.lp
sudo rpl --encoding UTF-8 "Additionally, you can\'t <code>CNAME</code> external domains \(<code>bing.com</code> to <code>google.com</code>\) successfully as this could result in invalid SSL certificate errors when the target server does not serve content for the requested domain." "erfolgreich <code>CNAME</code> verwenden, da dies zu Fehlern aufgrund ungültiger SSL-Zertifikate führen kann. Der Zielserver stellt keinen Inhalt für die angeforderte Domain bereit." /var/www/html/admin/settings-dnsrecords.lp
...
sudo rpl --encoding UTF-8 'Gives maximum amount of statistics' 'Liefert ein Maximum an Statistiken' /var/www/html/admin/settings-privacy.lp
...
sudo rpl --encoding UTF-8 'Warning:</strong><br>This archive contains sensitive information about your Pi-hole installation, e.g. your 2FA-TOTP secret \(if enabled\). Please be careful with this file and do not share it with anyone even if they claim to help you.' 'Warnung:</strong><br>Dieses Archiv enthält sensible Informationen über Ihre Pi-hole-Installation, z.B. Ihr 2FA-TOTP-Verschlüsselung (falls aktiviert). Bitte seien Sie vorsichtig mit dieser Datei und geben Sie sie nicht an andere weiter, auch wenn diese vorgeben, Ihnen zu helfen.' /var/www/html/admin/settings-teleporter.lp
sudo rpl --encoding UTF-8 'Warning:</strong><br>You are currently not using an end-to-end encryption. This means that secrets like your 2FA-TOTP secret will be transmitted in plain text. We recommend to use HTTPS when exporting your configuration.' 'Warnung:</strong><br>Sie verwenden derzeit keine Ende-zu-Ende-Verschlüsselung. Das bedeutet, dass Geheimnisse wie Ihre 2FA-TOTP-Verschlüsselung im Klartext übertragen wird. Wir empfehlen, HTTPS zu verwenden, wenn Sie Ihre Konfiguration exportieren.' /var/www/html/admin/settings-teleporter.lp
sudo rpl --encoding UTF-8 '<br>Export' '<br>Exportieren' /var/www/html/admin/settings-teleporter.lp
sudo rpl --encoding UTF-8 'Import previously exported configuration</h3>' 'Zuvor importierte Konfiguration importieren</h3>' /var/www/html/admin/settings-teleporter.lp
...
sudo rpl --encoding UTF-8 'Tools &rarr; Gravity</a> or run <code>pihole -g</code> to update your lists.' 'Werkzeuge &rarr; Gravity</a> oder im Terminal <code>pihole -g</code> um die Listen aktualisieren.' /var/www/html/admin/settings-teleporter.lp
...
sudo rpl --encoding UTF-8 'CPU:&nbsp' 'Prozessorlast:&nbsp' /var/www/html/admin/scripts/js/footer.js
sudo rpl --encoding UTF-8 '"Load: "' '"Geladen: "' /var/www/html/admin/scripts/js/footer.js
sudo rpl --encoding UTF-8 ' on "' ' auf "' /var/www/html/admin/scripts/js/footer.js
sudo rpl --encoding UTF-8 '" core" ' '" Kerne" ' /var/www/html/admin/scripts/js/footer.js
...
sudo rpl --encoding UTF-8 'Updating lists of ad-serving domains is not supported with this browser!' 'Das Aktualisieren von Listen mit Domains, die Werbung schalten, wird von diesem Browser nicht unterstützt!' /var/www/html/admin/scripts/js/gravity.js
...
sudo rpl --encoding UTF-8 '10, 25, 50, 100, "All"' '10, 25, 50, 100, "Alle"' /var/www/html/admin/scripts/js/queries.js
...
sudo rpl --encoding UTF-8 'Error while deleting CNAME record' 'Fehler beim Löschen des CNAME-Eintrags.' /var/www/html/admin/scripts/js/settings-dns-records.js
sudo rpl --encoding UTF-8 'in seconds <i>' 'in Sekunden <i>' /var/www/html/admin/scripts/js/settings-dns-records.js
...
sudo rpl --encoding UTF-8 'Importing Tricorder files is not supported with this browser!' 'Der Import von Tricoder-Dateien wird durch diesen Browser nicht unterstützt.' /var/www/html/admin/scripts/js/settings-teleporter.js
...
sudo rpl --encoding UTF-8 'Toggle navigation</span>' 'Navigation umschalten</span>' /var/www/html/admin/scripts/lua/header_authenticated.lp
