...
# getestet auf Pi-hole Version Core v6.4.3 - FTL Version v6.7 - Web Interface Version v6.6
# https://pi-hole.net/
#
...
sudo rpl --encoding UTF-8 'Address</th>' 'Addresse</th>' /var/www/html/admin/groups-lists.lp
...
sudo rpl --encoding UTF-8 'Hint:</strong> Click on the icons \(first and second columns\) to get additional information about your lists. The icons correspond to the health of the list.' 'Hinweis:</strong> Klicken Sie auf die Symbole (erste und zweite Spalte), um weitere Informationen zu Ihren Listen zu erhalten. Die Symbole geben den Status der jeweiligen Liste an.' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'List Status:</strong>' 'Listenstatus:</strong>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'List download was successful<br>' 'Herunterladen der Liste war erfolgreich<br>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'List unchanged upstream, Pi-hole used a local copy<br>' 'Die Liste blieb im Upstream unverändert, Pi-hole verwendete eine lokale Kopie<br><br>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'List unavailable, Pi-hole used a local copy<br>' 'Liste nich verfügbar, Pi-hole verwendet eine lokale Kopie<br>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'List unavailable and no local copy of this list is available<br>' 'Liste nicht verfügbar und es ist keine lokale Kopie dieser Liste verfügbar<br>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'Unknown<br>' 'Unbekannt<br>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'The status icon turns gray when the list is disabled.' 'Das Statussymbol wird grau, wenn die Liste deaktiviert ist.' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'Type:</strong>' 'Typ:</strong>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 'Allow list<br>' 'Erlaubte Liste<br>' /var/www/html/admin/groups-lists.lp
sudo rpl --encoding UTF-8 '</span> - Block list' '</span> - Blockierte Liste' /var/www/html/admin/groups-lists.lp
...
sudo rpl --encoding UTF-8 'Actions</th>' 'Aktionen</th>' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'Description:' 'Beschreibung:' /var/www/html/admin/settings-dhcp.lp
...
sudo rpl --encoding UTF-8 'Advanced Settings:' 'Erweiterte Einstellungen:' /var/www/html/admin/settings-dhcp.lp
...
sudo rpl --encoding UTF-8 'IPv6 addresses must be enclosed in square brackets like ' 'IPv6-Adressen müssen in eckige Klammern gesetzt werden, wie zum Beispiel ' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'Only letters, digits, hyphens and dots are allowed in hostnames.' 'In Hostnamen sind nur Buchstaben, Ziffern, Bindestriche und Punkte zulässig.' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'Examples:' 'Beispiele:' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'tells Pi-hole to give the machine with hardware address <code>00:20:e0:3b:13:af</code> the address <code>192.168.0.123</code></div>' 'Weist Pi-hole an, dem Gerät mit der Hardware-Adresse <code>00:20:e0:3b:13:af</code> die Adresse <code>192.168.0.123</code> zu geben.</div>' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'tells Pi-hole to give the machine with hardware address <code>00:20:e0:3b:13:af</code> the name <code>laptop</code>' 'Weist Pi-hole an, dem Gerät mit der Hardware-Adresse  <code>00:20:e0:3b:13:af</code> den Namen <code>Laptop</code> zu geben' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'tells Pi-hole to give the machine with hardware address <code>00:20:e0:3b:13:af</code> the address <code>192.168.0.123</code>, the name <code>laptop</code>, and an infinite DHCP lease' 'Weist Pi-hole an, dem Gerät mit der Hardware-Adresse <code>00:20:e0:3b:13:af</code> die Adresse <code>192.168.0.123</code>, den Namen <code>Laptop</code> und eine dauerhafte DHCP-Vergabezeit zu geben.' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'tells Pi-hole to give the machine with hardware address <code>00:20:e0:3b:13:af</code> the address <code>\[2001:db8::1\]' 'Weist Pi-hole an, dem Gerät mit der Hardware-Adresse <code>00:20:e0:3b:13:af</code> die Adresse <code>[2001:db8::1] zu geben.' /var/www/html/admin/settings-dhcp.lp
sudo rpl --encoding UTF-8 'Advanced settings help' 'Hilfe zu den erweiterten Einstellungen' /var/www/html/admin/settings-dhcp.lp
...
sudo rpl --encoding UTF-8 'Expand hostnames</strong>' 'Rechnernamen erweitern </strong>' /var/www/html/admin/settings-dns.lp
...
sudo rpl --encoding UTF-8 'The following list contains all reverse servers you want to add.</p>' 'Die folgende Liste enthält alle Reverse-Server, die Sie hinzufügen möchten.</p>' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'Enter reverse DNS servers, one per line' 'Geben Sie die Reverse-DNS-Server ein, einen pro Zeile' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'Enabled</th>' 'Aktiviert</th>' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 '<th>Network Range ' '<th>Netzwerkbereich ' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'Save the new entry' 'Neuen Eintrag speichern' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 '<label>Local network range using <a href="https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing" target="_blank">CIDR notation</a></label>' '<label>Lokaler Netzwerkbereich mit <a href="https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing" target="_blank">CIDR-Notation</a></label>' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 '<span class="help-block">If your local network spans 192.168.0.1 - 192.168.0.255, then you will have to input' '<span class="help-block">Wenn Ihr lokales Netzwerk den Bereich 192.168.0.1 bis 192.168.0.255 umfasst, müssen Sie Folgendes eingeben:' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 '<code>192.168.0.0/24</code>. If your local network is 192.168.47.1 - 192.168.47.255, it will be' '<code>192.168.0.0/24</code>. Wenn Ihr lokales Netzwerk den Bereich 192.168.47.1 bis 192.168.47.255 umfasst, lautet die Eingabe:' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 '<code>192.168.47.0/24</code> and similar. If your network is larger, the CIDR has to be  different,' '<code>192.168.47.0/24</code> usw. Wenn Ihr Netzwerk größer ist, muss der CIDR-Block anders sein.' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'for instance a range of 10.8.0.1 - 10.8.255.255 results in <code>10.8.0.0/16</code>, whereas an even' 'Beispielsweise ergibt ein Bereich von 10.8.0.1 bis 10.8.255.255 <code>10.8.0.0/16</code>, während ein noch größeres Netzwerk von' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'wider network of 10.0.0.1 - 10.255.255.255 results in <code>10.0.0.0/8</code>. Setting up IPv6 ranges' '10.0.0.1 bis 10.255.255.255 <code>10.0.0.0/8</code> ergibt.' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'is exactly similar to setting up IPv4 here and fully supported. Feel free to reach out to us on our' 'Das Einrichten von IPv6-Bereichen ist genau wie das Einrichten von IPv4 und wird vollständig unterstützt.' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 '<a href="https://discourse.pi-hole.net" rel="noopener noreferrer" target="_blank">Discourse forum</a>' 'Bei Fragen zur Einrichtung der lokalen Hostnamensauflösung für Ihr System können Sie sich gerne an uns wenden:' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'in case you need any assistance setting up local host name resolution for your particular system.</span>' '<a href="https://discourse.pi-hole.net" rel="noopener noreferrer" target="_blank">Diskussionsforum</a>' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'IP address of your DHCP server \(or router\)</label>' 'IP Addresse Ihres DHCP-Servers (oder Routers)</label>' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'Local domain name \(optional\)' 'Lokaler Domain Name (optional)' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'You can also specify a local domain name \(like <code>fritz.box</code>\) to ensure queries to' 'Sie können auch einen lokalen Domänennamen angeben (z. B. <code>fritz.box</code>), um sicherzustellen,' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'devices ending in your local domain name will not leave your network, however, this is optional.' 'dass Anfragen an Geräte, die mit Ihrem lokalen Domänennamen enden, Ihr Netzwerk nicht verlassen. Dies ist jedoch optional.' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'The local domain name must match the domain name specified in your DHCP server for this to work.' 'Damit dies funktioniert, muss der lokale Domänenname mit dem im DHCP-Server angegebenen Domänennamen übereinstimmen.' /var/www/html/admin/settings-dns.lp
sudo rpl --encoding UTF-8 'You can likely find it within the DHCP settings.' 'Sie finden es wahrscheinlich in den DHCP-Einstellungen.' /var/www/html/admin/settings-dns.lp
